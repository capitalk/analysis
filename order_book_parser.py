...
import sys
from order_book import Order, OrderBook, Action 
from order_book import ADD_ACTION_TYPE, MODIFY_ACTION_TYPE, DELETE_ACTION_TYPE
from order_book import BID_SIDE, OFFER_SIDE 
import orderBookConstants_v1 as obc 
...
def books_from_lines_v1(lines, debug=False, end=None, drop_out_of_order=False):
    ...
    for line in lines:
        ...
        if line[0:9] == "ORDERBOOK":
            ...
            currBook = OrderBook(day = None, last_update_time = lastUpdateTime,
              last_update_monotonic = None, bids = [], offers = [], actions = [])
        else: 
            row = line.split(',')
            side = row[obc.SIDE]
            entry = Order(
...
class V3_Parser:
    ...
    def parse_add_action(self, line):
        ...
        action = Action(action_type, side, price, volume) 
    ...
    def start_new_orderbook(self, line):         
        ...
        self.currBook = OrderBook(exchange_day, update_time, monotonic_time, [], [], self.actions)
        self.actions = []
          ...
          try:
              ...
              if line[0] == 'Q':  
                  ...
                  if order.side == OFFER_SIDE:
                    self.currBook.offers.append(order)
                  else:
                    self.currBook.bids.append(order)
              else:
